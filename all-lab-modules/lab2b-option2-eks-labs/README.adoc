= Deploy your container onto EKS

Deploying individual containers is not difficult. However, when you need to coordinate many container deployments, a container management tool like EKS can greatly simplify this.

During the first step (lab environment setup) you have already setup the EKS cluster.
...
----
+
3. Open the *mythicalmysfits-eks-manifest.yaml* file by double clicking the filename
in the lefthand navigation in Cloud9.

4. The file has the following contents:
+
.mythical-mysfits-eks-manifest.yaml
[source,yaml]
----
apiVersion: v1
kind: List
...
- apiVersion: v1
  kind: Namespace
  metadata:
    name: mythicalmysfitseks

- kind: PersistentVolume
  apiVersion: v1
  metadata:
    name: mysfits-pv-volume
    namespace: mythicalmysfitseks
    labels:
      type: local
  spec:
